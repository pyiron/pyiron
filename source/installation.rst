...
The pyiron IDE is designed to scale from single workstations to heterogeneous computing infrastructures. The principles are the same, still to leverage the scalability of a computer cluster, it is reasonable to setup separate database servers or login nodes. 
...
Linux - Cluster
===============
Based on the Linux workstation installation, we replace the Jupyter notebook server with Jupyterhub and the file-based SQLite database with Postgres database for scalability. For more details please contact the developers. 
