...
def _get_value_from_incar(job, key):
    return eval(job['input/incar/data_dict']['Value'][job['input/incar/data_dict']['Parameter'].index(key)])
...
def _get_majority(lst, minority=False):
    ...
    majority_element = [key for key, value in elements_dict.items() if value == max_value][0]
    if minority:
...
def get_incar(job):
    ...
    return {key: value for key, value in zip(data_dict['Parameter'],
                                             data_dict['Value'])}
...
def get_majority_species(job):
    ...
    majority_element, minority_lst = _get_majority([element_lst[ind] for ind in indices_lst],
                                                   minority=True)
...
def get_majority_crystal_structure(job):
    ...
    majority_index = [ind for ind, el in enumerate(basis) if el.symbol == majority_element]
...
def get_equilibrium_parameters(job):
    return {key: job['output/' + key] for key in ['equilibrium_energy',
                                                  'equilibrium_b_prime',
...
def get_structure(job):
    ...
    atoms_dict = {'symbols': atoms.get_chemical_symbols(),
