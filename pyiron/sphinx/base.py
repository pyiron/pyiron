...
class SphinxBase(GenericDFTJob):
    ...
    def set_species_group(self, check_overlap=True, potformat='JTH'):
        ...
        for species_obj in self.structure.get_species_objects():
            ...
            self.input.species.setdefault("species", [])
            if potformat == "JTH":
                self.input.species["species"].append(
                    odict(
                        [
                            ("name", '"' + elem + '"'),
                            ("potType", '"AtomPAW"'),
    ...
    def set_main_group(self):
        ...
        if self.input["Istep"] is not None:
            ...
            if self.input["dE"] is not None:
                self.input.main["linQN"]["dEnergy"] = str(
            ...
            if self._generic_input["restart_for_band_structure"]:
                ...
                self.input.main["scfDiag"].append(self.get_scf_group())
    ...
    def set_guess_group(self):
        ...
        self.input.guess.setdefault("waves", Group())
        self.input.guess.waves.setdefault("lcao", Group())
        ...
        if charge_density_file is None:
            self.input.guess.setdefault("rho", Group({"atomicOrbitals": True}))
        else:
    ...
    def restart(
        ...
        if from_charge_density and os.path.isfile(
            ...
            new_job.input.guess.remove_flag("atomicOrbitals")
    ...
    def write_input(self):
        ...
        self.input_writer.write_structure(self.input.structure,
            cwd=self.working_directory, file_name="structure.sx")

        # self.input.basis --> basis.sx
        self.input_writer.write_group(self.input.basis,
            cwd=self.working_directory, file_name="basis.sx")

        # self.input.hamilton --> PAWHamiltonian.sx
        self.input_writer.write_group(self.input.hamilton,
            cwd=self.working_directory, file_name="PAWHamiltonian.sx")

        # self.input.guess --> initialGuess.sx
        self.input_writer.write_group(self.input.guess,
            cwd=self.working_directory, file_name="initialGuess.sx")
        ...
        self.input_writer.write_group(self.input.spin,
            cwd=self.working_directory, file_name="spinConstraint.sx")

        # self.input.main --> main.sx
        self.input_writer.write_group(self.input.main,
            cwd=self.working_directory, file_name="main.sx")

        # Write control wrapper (primary SPHInX input.sx)
        self.input_writer.write_wrapper(job_name=self.job_name,
            cwd=self.working_directory, enable_kjxc=self.input["KJxc"])
    ...
    def check_setup(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if not (
                isinstance(self.input.basis["kPoint"]["coords"], list)
                or len(self.input.basis["kPoint"]["coords"]) != 3
            ):
...
class InputWriter(object):
    ...
    def write_wrapper(self, job_name, cwd=None, file_name="input.sx", enable_kjxc=False):
        """
    ...
    def write_group(self, group, cwd, file_name):
        file_name = posixpath.join(cwd, file_name)
        with open(file_name, "w") as f:
            f.write(group.to_sx_str())

    def write_species(self, group, cwd, file_name):
        file_name = posixpath.join(cwd, file_name)
        with open(file_name, "w") as f:
            for i in range(len(group["species"])):
                f.write("species {\n")
                for k, v in group["species"][i].items():
                    f.write(f"\t{k} = {v};\n")
                f.write("}\n")

    def write_structure(self, group, cwd, file_name):
        file_name = posixpath.join(cwd, file_name)
        with open(file_name, "w") as f:
