...
class SphinxInteractive(SphinxBase, GenericInteractive):
    ...
    def interactive_energy_pot_getter(self):
        ...
        return np.array(float(self._interactive_library_read.readline()) * HARTREE_TO_EV)
    ...
    def interactive_forces_getter(self):
        ...
        for _ in range(len(self.structure)):
            ...
            ff.append([float(line[i])*HARTREE_OVER_BOHR_TO_EV_OVER_ANGSTROM for i in range(3)])
    ...
    def interactive_initialize_interface(self):
        ...
        if self.server.cores == 1 or not self.executable.mpi:
            out = subprocess.Popen(str(self.executable), cwd=self.project_hdf5.working_directory, shell=True,
                                   stderr=subprocess.STDOUT, universal_newlines=True)
        else:
            out = subprocess.Popen([self.executable.executable_path, str(self.server.cores), str(self.server.threads)],
        ...
        self._interactive_library = open(os.path.join(self.working_directory, 'sxctrl'), 'w')
        self._interactive_library_read = open(os.path.join(self.working_directory, 'sxres'), 'r')
        self._logger.debug('interactive interface is opened!')
        if np.all(self.structure.get_initial_magnetic_moments()==None) and 'atom_spins' in self.interactive_cache.keys():
                        ...
                        with hdf5_input.open("generic") as hdf5_generic:
                            ...
                            with hdf5_generic.open("dft") as hdf5_dft:
                                if "atom_spin_constraints" in h5['interactive'].list_nodes():
                                    hdf5_dft["atom_spin_constraints"] = h5['interactive/atom_spin_constraints']
    ...
    def calc_minimize(self, electronic_steps=None, ionic_steps=None, max_iter=None, pressure=None, algorithm=None,
        ...
        if self.server.run_mode.interactive or self.server.run_mode.interactive_non_modal:
            ...
            super(SphinxInteractive, self).calc_minimize(electronic_steps=electronic_steps, ionic_steps=ionic_steps,
    ...
    def _interactive_pipes_initialized(self):
        return (os.path.exists(os.path.join(self.working_directory, 'sxctrl')) and
                os.path.exists(os.path.join(self.working_directory, 'sxres')))
    ...
    def calc_static(self, electronic_steps=400, blockSize=8, dSpinMoment=1e-8, algorithm=None,
                    retain_charge_density=False, retain_electrostatic_potential=False):
        """
        ...
        """
        super(SphinxInteractive, self).calc_static(electronic_steps=electronic_steps, algorithm=algorithm,
                                                   retain_charge_density=retain_charge_density,
                                                   retain_electrostatic_potential=retain_electrostatic_potential)
    ...
    def _control_str(self):
        control_str = odict() 
        if self.server.run_mode.interactive or self.server.run_mode.interactive_non_modal:
            commands = [odict([('id', '"restart"'),
                               ('scfDiag', self._input_control_scf_string(maxSteps=10, keepRhoFixed=True, dEnergy=1.0e-4))])]
            commands.append(odict([('id', '"coarseelectronicminimization"'),
                                   ('scfDiag', self._input_control_scf_string(dEnergy='1000*Ediff/'+str(HARTREE_TO_EV)))]))
            commands.append(odict([('id', '"electronicminimization"'), ('scfDiag', self._input_control_scf_string())]))
