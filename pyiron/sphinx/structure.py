...
def read_atoms(filename='structure.sx'):
    ...
    for i, line in enumerate(file_string):
        ...
        if species_trigger in line:
            species = line.strip().split("=")[-1].replace(";", "").replace('"', '').strip()
            ...
            for j in range(len(file_string)-i):
                ...
                if 'atom' in line_str:
                    ...
                    while not break_loop:
                        position_string = " ".join(file_string[i + j + k].split("=")[-1])
                        replace_list = ["[", "]", ";", "}"]
                        for rep in replace_list:
                            position_string = "".join(position_string).replace(rep, " ").split()
                        positions.append(np.array(position_string[0].split(","), dtype=float))
                        positions_dict[species] += 1
                        k += 1
                        if (i + j + k) <= len(file_string) - 1:
                            if "element" in file_string[i + j + k] or "atom" not in file_string[i + j + k]:
                                break_loop = True
                    break
    indices = list()
    ...
    atoms = Atoms(species=species_list, indices=indices, cell=cell, positions=np.array(positions) * BOHR_TO_ANGSTROM)
    return atoms
