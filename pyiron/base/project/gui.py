...
class ProjectGUI:
    ...
    def dict_to_str(my_dict):
        ...
        if 'Parameter' in my_dict.keys():
            ...
            table = ['{}: {} {} {}'.format(key, val, com, eol) for key, val, com in
                     zip(key, val, com)]
        else:
            table = ['{}: {} {}'.format(key, val, eol) for key, val in
                     my_dict.items()]
        return ''.join(table)
    ...
    def refresh_view(self):
        ...
        if isinstance(self.project, str):
            ...
            if len(self.project) < max_length:
                ...
                self.w_text.value = '<br>'.join(self.project[:max_length]) + eol + ' .... file too long: skipped ....'
            ...
            groups = sorted([el for el in self.project.list_groups() if not is_h5_dir(el)])
            ...
            if hasattr(self.project, 'list_files'):
                files = sorted([el for el in self.project.list_files() if not is_h5_file(el)])
