...
class SxExtOpt(InteractiveInterface):
    def __init__(self, structure, working_directory=None, maxDist=5, ionic_steps=1000, ionic_energy=1.0e-3,
                 ionic_forces=1.0e-2, max_step_length=1.0e-1, soft_mode_damping=1, executable=None):
        ...
        if executable is None:
            executable = Executable(path_binary_codes=s._configuration['resource_paths'],
                                    codename="SxExtOptInteractive",
                                    module=self.__module__.split('.')[1],
                                    overwrite_nt_flag=False).executable_path
        self._start_process(structure=structure,
    ...
    def _start_process(self, structure, executable, maxDist=5, ionic_steps=1000, ionic_energy=1.0e-3, ionic_forces=1.0e-2,
                       max_step_length=1.0e-1, soft_mode_damping=1, selective_dynamics=False):
        if selective_dynamics:
            input_writer_obj = InputWriter()
            input_writer_obj.structure = structure
            input_writer_obj.write_structure(file_name='structure.sx',
                                             cwd=self.working_directory,
                                             structure_str=None,
                                             symmetry_enabled=True,
                                             keep_angstrom=True)
        self._write_input(working_directory=self.working_directory,
                          maxDist=maxDist,
                          ionic_steps=ionic_steps,
                          ionic_energy=ionic_energy,
                          ionic_forces=ionic_forces,
        ...
        shell = (os.name == 'nt')
        try:
            with open(posixpath.join(self.working_directory, 'out.txt'), mode='w') as f_out:
                with open(posixpath.join(self.working_directory, 'error.txt'), mode='w') as f_err:
                    self._process = subprocess.Popen([executable],
                                                     cwd=self.working_directory,
                                                     shell=shell, stdout=f_out, stderr=f_err,
    ...
    @staticmethod
    def _write_input(working_directory, maxDist=5, ionic_steps=1000, ionic_energy=1.0e-3, ionic_forces=1.0e-2,
                     max_step_length=1.0e-1, soft_mode_damping=1, selective_dynamics=False):
        with open(os.path.join(working_directory, 'optim.sx'), 'w') as f:
            content = ('main { ricQN { ric { maxDist = %f; withAngles; } maxSteps = %i; dEnergy = %f; dF = %f; maxStepLength = %f; softModeDamping = %f;}}'
                       % (maxDist, ionic_steps, ionic_energy, ionic_forces, max_step_length, soft_mode_damping))
            if selective_dynamics:
    ...
    def _interactive_pipes_initialized(working_directory):
        return (os.path.exists(os.path.join(working_directory, 'control')) and
                os.path.exists(os.path.join(working_directory, 'response')))
    ...
    def _interactive_initialize_interface(self):
        self._interactive_library_read = open(os.path.join(self.working_directory, 'control'), 'r')
        self._interactive_library = open(os.path.join(self.working_directory, 'response'), 'w')
    ...
    def _write_positions(self, positions, elements):
        for pos, el in zip(positions, elements):
            self._interactive_write_line('%.16f %.16f %.16f %s' % (pos[0], pos[1], pos[2], str(el)))
    ...
    def _read_positions(self, count):
        return [[float(c) for c in self._interactive_library_read.readline().split()]
    ...
    def set_forces(self, forces):
        ...
        if len(line) == 0 or line[0] == 'end':
            ...
            if line[1] == 'cell':
                ...
                self._write_positions(positions=self._positions, elements=self._elements)
            elif line[1] == 'energy':
                self._write_energy()
            elif line[1] == 'forces':
...
class SxExtOptInteractive(InteractiveWrapper):
    ...
    def run_static(self):
        ...
        self._interactive_interface = SxExtOpt(structure=self.ref_job.structure,
        ...
        while self._interactive_number_of_steps < self.input['ionic_steps'] and \
                not self._interactive_interface.converged:
            ...
            if self.ref_job.server.run_mode.interactive:
                ...
                self.ref_job.run()
                if self._coarse_run and np.max(np.linalg.norm(self.get_forces(), axis=-1), axis=-1)<self.input['ionic_forces']:
                    self._coarse_run = False
