...
class TestSphinxStructure(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.file_list = [posixpath.join(self.file_location, structure_directory, f) for f in file_list]
    ...
    def test_read_atoms(self):
        for i, f in enumerate(self.file_list):
            ...
            if i == 0:
                ...
                self.assertTrue(np.allclose(atoms.cell / BOHR_TO_ANGSTROM,
                                            [[18.0936435257, 0.0, 0.0], [-12.0624290171, 20.8927399203, 0.0],
                                             [0.0, 0.0, 39.1932378013]]))
            if i == 1:
                self.assertEqual(atoms.get_chemical_formula(), 'C2Mg5')
                self.assertTrue(np.allclose(atoms.cell / BOHR_TO_ANGSTROM, [[18.09364353, 0.00000000, 0.00000000],
